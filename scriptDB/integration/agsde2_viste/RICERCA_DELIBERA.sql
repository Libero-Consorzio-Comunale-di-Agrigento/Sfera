...
--changeset rdestasio:install_20210221_viste_03 runOnChange:true
...
SELECT                                 -- dati identificativi del documento
         prop.id_proposta_delibera AS ID_DOCUMENTO,
          DECODE (deli.id_delibera, NULL, 'PROPOSTA_DELIBERA', 'DELIBERA')
             AS TIPO_DOCUMENTO,
          ...
          CAST (NVL (deli.riservato, prop.riservato) AS CHAR (1))
             AS RISERVATO,
          UPPER (
             DECODE (
          ...
          -- so4_util.anuo_get_descrizione (pr_uo_prop.unita_progr,  pr_uo_prop.unita_dal) AS UO_PROPONENTE_DESCRIZIONE,
            ...
            WHERE     progr = pr_uo_prop.unita_progr
                  AND ottica = pr_uo_prop.unita_ottica
                  AND dal = pr_uo_prop.unita_dal)
          ...
          NVL (de_attori.unita_progr, pr_attori.unita_progr)
             AS STEP_UNITA_PROGR,
          NVL (de_attori.unita_dal, pr_attori.unita_dal) AS STEP_UNITA_DAL,
          NVL (de_attori.unita_ottica, pr_attori.unita_ottica)
             AS STEP_UNITA_OTTICA,
          NVL (de_attori.ruolo, pr_attori.ruolo) AS STEP_RUOLO,
          NVL (de_cfg_step.titolo, pr_cfg_step.titolo) AS STEP_NOME,
          NVL (de_cfg_step.descrizione, pr_cfg_step.descrizione)
             AS STEP_DESCRIZIONE,
          -- dati dell'atto
          ...
          CAST ('' AS DATE) AS DATA_SCADENZA,
          -- dati della seconda numerazione
          CAST (NULL AS NUMBER) AS NUMERO_ATTO_2,
          CAST (NULL AS NUMBER) AS ANNO_ATTO_2,
          CAST ('' AS VARCHAR2 (255)) AS REGISTRO_ATTO_2,
          CAST (NULL AS DATE) AS DATA_NUMERO_ATTO_2,
          -- dati della proposta
          ...
          WHERE     j.id_documento_rif = deli.id_documento_esterno
                AND NOT EXISTS
                           (SELECT 1
          ...
          jcons_pkg.get_data_conservazione (deli.id_documento_esterno) AS DATA_CONSERVAZIONE,
          ...
          NVL (deli.id_file_allegato_testo, prop.id_file_allegato_testo)
             AS ID_TESTO,
          af.id_file AS ID_FILE_ALLEGATO,
          prop.id_oggetto_ricorrente ID_OGGETTO_RICORRENTE,
          deli.stato_marcatura STATO_MARCATURA,
          deli.stato_firma STATO_FIRMA,
          prop.data_ordinamento DATA_ORDINAMENTO,
          rel.ni_assessore NI_RELATORE,
          CAST (utility_pkg.get_delibera_conclusa(deli.id_delibera) AS CHAR (1)) AS ATTO_CONCLUSO,
          NULL CUP,
    ...
    WHERE     pr_sog.id_proposta_delibera(+) = prop.id_proposta_delibera
          ...
          AND af.id_allegato(+) = de_alle.id_allegato
          AND deli.valido = 'Y'
          AND rel.id_delega(+) = prop.id_delega
          AND rel.valido(+) = 'Y'
          AND b.id_proposta_delibera(+) = prop.id_proposta_delibera
   UNION ALL
  SELECT                                 -- dati identificativi del documento
         prop.id_proposta_delibera AS ID_DOCUMENTO,
          DECODE (deli.id_delibera, NULL, 'PROPOSTA_DELIBERA', 'DELIBERA')
          ...
          pr_sog.unita_ottica AS UO_PROPONENTE_OTTICA,
            ...
            WHERE     progr = pr_uo_prop.unita_progr
                  ...
                  AND dal = pr_uo_prop.unita_dal)
             AS UO_PROPONENTE_DESCRIZIONE,
          DECODE (deli.id_delibera, NULL, pr_sog.utente, ds.utente) AS UTENTE_SOGGETTO,
          DECODE (deli.id_delibera, NULL, pr_sog.tipo_soggetto, ds.tipo_soggetto) AS TIPO_SOGGETTO,
          -- competenze
          de_comp.id_delibera_competenza AS ID_COMPETENZA,
          de_comp.utente AS COMP_UTENTE,
          de_comp.unita_progr AS COMP_UNITA_PROGR,
     ...
     FROM proposte_delibera prop,
          ...
          deleghe rel,
          budget b
    WHERE     ds.id_delibera(+) = deli.id_delibera
   ...
   SELECT                                 -- dati identificativi del documento
         prop.id_proposta_delibera AS ID_DOCUMENTO,
          ...
          -- corte dei conti
          CAST (
             NVL (deli.da_inviare_corte_conti, prop.da_inviare_corte_conti) AS CHAR (1))
             AS DA_INVIARE_CORTE_CONTI,
          TRUNC (deli.data_invio_corte_conti) AS DATA_INVIO_CORTE_CONTI,
          -- dati specifici della ricerca delibera
          prop.id_commissione AS ID_COMMISSIONE,
          deli.id_oggetto_seduta AS ID_OGGETTO_SEDUTA,
          ...
          deli.stato_marcatura STATO_MARCATURA,
          deli.stato_firma STATO_FIRMA,
          prop.data_ordinamento AS DATA_ORDINAMENTO,
          rel.ni_assessore NI_RELATORE,
          CAST (utility_pkg.get_delibera_conclusa(deli.id_delibera) AS CHAR (1)) AS ATTO_CONCLUSO,
          NULL CUP,
          b.conto_economico CONTO_ECONOMICO,
    ...
    WHERE     ds.id_delibera (+) = deli.id_delibera
          ...
          AND af.id_allegato(+) = pr_alle.id_allegato
          AND prop.valido = 'Y'
          AND rel.id_delega(+) = prop.id_delega
          AND rel.valido(+) = 'Y'
          AND b.id_proposta_delibera(+) = prop.id_proposta_delibera
