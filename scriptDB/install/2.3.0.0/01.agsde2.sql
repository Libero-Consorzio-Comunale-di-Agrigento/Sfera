...
--changeset rdestasio:2.3.0.0_20200221_01
 ...
 * VERSIONE 2.3.0.x
 */
ALTER TABLE DETERMINE ADD (NUMERO_DETERMINA_2       NUMBER(10),
                           ANNO_DETERMINA_2         NUMBER(10),
                           REGISTRO_DETERMINA_2     VARCHAR(255 BYTE),
                           DATA_NUMERO_DETERMINA_2  DATE)
/

ALTER TABLE TIPI_DETERMINA ADD (ID_TIPO_REGISTRO_2 VARCHAR (255 BYTE))
/

create unique index determine_numero_2_dete_uk
   on determine (anno_determina_2
               , nvl2 ("NUMERO_DETERMINA_2", "ENTE", null)
               , numero_determina_2
               , registro_determina_2)
/

-- gestione del soggetto incaricato: http://svi-redmine/issues/22273
INSERT INTO TIPI_SOGGETTI (CATEGORIA, CODICE, TITOLO, DESCRIZIONE) VALUES ( 'COMPONENTE', 'INCARICATO', 'Incaricato', 'Incaricato')
/

-- gestione delle note di trasmissione per i visti / pareri
ALTER TABLE VISTI_PARERI ADD (NOTE_TRASMISSIONE VARCHAR (4000 BYTE))
/

ALTER TABLE VISTI_PARERI_STORICO ADD (NOTE_TRASMISSIONE VARCHAR (4000 BYTE))
/

-- correzione del campo indirizzi email di sedute notifiche da varchar2(4000) a CLOB
ALTER TABLE odg_sedute_notifiche ADD (tmpdetails  CLOB)
/

UPDATE odg_sedute_notifiche SET tmpdetails = indirizzi_email
/

ALTER TABLE odg_sedute_notifiche DROP COLUMN indirizzi_email
/

ALTER TABLE odg_sedute_notifiche RENAME COLUMN tmpdetails TO indirizzi_email
/
